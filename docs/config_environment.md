...
| `GROUPING_MERGE_DISTANCE` #env | float | `0.35` | `MapReduceGrouping` fragment-merge cosine-distance cutoff over map-summary embeddings; tighter than the article-level threshold, tune with the evaluation harness ([settings.py:68](../src/news/settings.py#L68)) |
