...
| `miniflux_client.py` | `MinifluxClient` HTTP client: category lookup, paginated entry fetching, per-link deduplication of repeated captures, auth header, transient-error retries; `miniflux_client` factory context manager ([src/news/digest/miniflux_client.py:21-270](../src/news/digest/miniflux_client.py#L21-L270)) |
| `llm_client.py` | `LlmClient` OpenAI-compatible wrapper: `chat` (raw text), `chat_parsed` (structured output), and `embeddings` with retries; `llm_client` factory context manager ([src/news/digest/llm_client.py:24-138](../src/news/digest/llm_client.py#L24-L138)) |
...
| `map_reduce.py` | `MapReduceGrouping`, a second `Grouping` implementation: embeds one title+content text per entry in token-capped, concurrency-limited batches, L2-normalizing the result via `sklearn.preprocessing.normalize` (`embed_entries`); `calibrate_threshold` samples a cosine-distance cutoff from the corpus's random-pair similarity baseline; `cluster` runs average-linkage `AgglomerativeClustering` with no noise class; `score_clusters` ranks every group using hostname entropy, corpus-relative arrival bursts, and optional historical-embedding novelty. `__call__` selects the top `grouping_map_clusters` scored clusters, maps each to a title/summary/entities with one concurrency-limited `chat_parsed` call per cluster (falling back to the cluster's top entry title on failure), then merges fragments of one event in Python (`_merge_fragments`): embeds `title + entities + summary` per map output, clusters them with average-linkage HAC at `grouping_merge_distance`, unions each bucket's entries and re-scores the pooled records via `compute_metrics` (the single trend-score definition, shared with `score_clusters` against one `RateBaseline`). No LLM call decides which stories are the same event. The result is ranked by `trend_score` and truncated to `grouping_max_records` ([src/news/digest/map_reduce.py](../src/news/digest/map_reduce.py)) |
