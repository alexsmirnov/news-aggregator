...
1. **Fetch:** `DigestService.fetch_entries` resolves the Miniflux category and paginates entries published within the lookback window, strips HTML with BeautifulSoup, and truncates content ([src/news/digest/service.py:88-126](../src/news/digest/service.py#L88-L126)).
2. **Group:** formatted entries are sent to the grouping model as structured output (`response_format=NewsResponse`, `reasoning_effort="high"`), producing `NewsRecord` items of a combined headline plus source links ([src/news/digest/service.py:128-165](../src/news/digest/service.py#L128-L165), prompts in [src/news/digest/prompts.py:8-33](../src/news/digest/prompts.py#L8-L33)).
3. **Refine:** for each group, matched entry content plus up to `refine_max_links` links are summarized by the refinement model with the `url_context` tool so the model can read linked pages directly; refinements run concurrently capped at 8 ([src/news/digest/service.py:167-295](../src/news/digest/service.py#L167-L295)).
4. **Persist:** the `Digest` JSON is written to `<digest_output_dir>/YYYY/MM/<name>-DD.json` ([src/news/digest/service.py:297-319](../src/news/digest/service.py#L297-L319), path layout in [src/news/digest/archive.py:27-33](../src/news/digest/archive.py#L27-L33)).
...
Aggregations run sequentially per pipeline run; a failing aggregation is logged and skipped without aborting the others ([src/news/digest/service.py:371-390](../src/news/digest/service.py#L371-L390)). Default aggregations map names to Miniflux categories with topic focus texts: `news`, `economy`, `technology` ([src/news/settings.py:39-43](../src/news/settings.py#L39-L43)).
